...
def Reload_Predict_TF1(loadmodel_path ,input_tensordata_Mat ):
    ...
    with graph.as_default():
        ...
        #output_ =model(input_tensor ,training=False )
...
def  model_save_TF1 (load_model_path_fromTF2 ,export_path ,input_tensordata_Mat) :
    ...
    with graph.as_default():
        ...
        output_  =model(input_tensor ,training=False )  # must
    #    print( 'TF1_pred_output=',output_)
        ...
        with tf.Session(graph=graph ) as sess:
            ...
            if loadd_model_path.find('TF1')<0:  #when load TF2 savemodel  ,to save for TF1
                ...
                #save method1
                #tf.saved_model.save(model, export_path)   # savemodel reloaded  :pass
                
                #save method2
                tf.saved_model.simple_save( session= sess , export_dir=export_path , inputs=inputs_info, outputs=outputs_info  )
